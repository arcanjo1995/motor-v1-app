...
with aba_tipo_b:
    ...
    if st.button("🚀 Executar Releituras e Gerar Sinal"):
        if not entrada_numeros: st.error("Erro: Campo de entrada vazio.")
        else:
            try:
                lista_numeros = [int(x.strip()) for x in entrada_numeros.split(",")]
                if len(lista_numeros) != 12: st.error("Erro: É necessário exatamente 12 números.")
                else:
                    processador = ProcessadorTipoB(lista_numeros, NOME_BASE_DEFINITIVA)
                    resultado = processador.executar_sinal_real()
                    
                    if "erro" in resultado: st.error(resultado["erro"])
                    else:
                        st.success(f"**SINAL GERADO:** {resultado['sinal']}")
                        col1, col2 = st.columns([2, 1])
                        with col1: st.write(f"**Justificativa:** {resultado['justificativa']}")
                        with col2: st.metric("Confiança da IA", f"{resultado['confianca_ia']}%")
                        ...
                        if resultado.get("motivo_real"): st.caption(f"Motivo real da decisão: {resultado['motivo_real']}")
    ...
    if arquivo_upload is not None:
        ...
        if os.path.exists(NOME_BASE_DEFINITIVA):
            try:
                ...
                if dados_existentes: registros_atuais = len(dados_existentes)
            except: registros_atuais = 0

        if registros_atuais > 0: st.info(f"📁 **Base de Longo Prazo atual tem {registros_atuais} registros.**")
        else: st.warning("📁 Ainda não existe uma Base de Longo Prazo salva.")

        col1, col2, col3 = st.columns(3)
        with col1: rodar_auditoria = st.button("🔍 Iniciar Auditoria de Recência")
        with col2: salvar_como_base = st.button("💾 Substituir Base de Longo Prazo")
        with col3: adicionar_base = st.button("➕ Adicionar à Base de Longo Prazo")
        ...
        if rodar_auditoria:
            ...
            if dados:
                ...
                if hasattr(ia, 'analise_recencia') and ia.analise_recencia:
                    with st.expander("🔬 Análise de Comportamento na RECÊNCIA (pós-número)", expanded=False): st.json(ia.analise_recencia)
            ...
            else: st.error("IMPOSSÍVEL CALCULAR - Estrutura fora do padrão do Volume 8.")
                ...
                if dados:
                    ...
                    if not relatorio.get("modelo_salvo_com_sucesso", False):
                        try:
                            ...
                            if ia_para_salvar and salvar_modelo_longo_prazo(ia_para_salvar): relatorio["modelo_salvo_com_sucesso"] = True
                        except: pass
                    
                    if relatorio.get("sucesso"):
                        if relatorio.get("modelo_salvo_com_sucesso"): st.success("✅ Base de Longo Prazo substituída e treinada com sucesso!")
                        else: st.error("Modelo ainda não foi salvo corretamente. Tente reiniciar o app.")
                        ...
                        with col2:
                            st.metric("G0", relatorio["G0"]); st.metric("G1", relatorio["G1"]); st.metric("G2", relatorio["G2"])
                        with col3:
                            st.metric("Falhas", relatorio["FALHA"]); st.metric("NO CALL", relatorio["NO CALL"]); st.metric("Regras Boas", relatorio["regras_com_boa_performance"])
                        ...
                        if "analise_comportamento_numeros" in relatorio:
                            with st.expander("🔬 Análise Detalhada por Número (Comportamento Pós-Aparição)", expanded=False): st.json(relatorio["analise_comportamento_numeros"])
                                ...
                                if hasattr(ia_atual, 'padroes_xadrez_detalhado') and ia_atual.padroes_xadrez_detalhado:
                                    for padrao, info in ia_atual.padroes_xadrez_detalhado.items():
                                        if info.get("total", 0) >= 8:
                                            ...
                                            st.json({"Após → Vermelho": info.get("apos_V", 0), "Após → Preto": info.get("apos_P", 0), "Números que trollam": {str(num_k): qtd_v for num_k, qtd_v in info.get("quebradores", {}).items()}, "G0": info.get("g0", 0), "G1": info.get("g1", 0), "Assertividade %": round(((info.get("g0", 0) + info.get("g1", 0)) / info.get("total", 1)) * 100, 1)})
                                else: st.info("Nenhum padrão de Xadrez relevante encontrado.")
                                ...
                                if hasattr(ia_atual, 'padroes_streak_detalhado') and ia_atual.padroes_streak_detalhado:
                                    for padrao, info in ia_atual.padroes_streak_detalhado.items():
                                        if info.get("total", 0) >= 8:
                                            st.markdown(f"**{padrao}** — {info.get('total')}x")
                                            st.json({"Após → Vermelho": info.get("apos_V", 0), "Após → Preto": info.get("apos_P", 0), "Números que trollam": {str(num_k): qtd_v for num_k, qtd_v in info.get("quebradores", {}).items()}, "G0": info.get("g0", 0), "G1": info.get("g1", 0), "Assertividade %": round(((info.get("g0", 0) + info.get("g1", 0)) / info.get("total", 1)) * 100, 1)})
                                else: st.info("Nenhum padrão de Streak relevante encontrado.")
                            else: st.error("Modelo ainda não foi salvo corretamente. Tente novamente ou reinicie o app.")
                                ...
                                if hasattr(ia_atual, 'padroes_gerais_detalhado') and ia_atual.padroes_gerais_detalhado:
                                    for padrao, info in ia_atual.padroes_gerais_detalhado.items():
                                        if info.get("total", 0) >= 5:
                                            st.markdown(f"**{padrao}** — {info.get('total')}x")
                                            st.json({"Após → Vermelho": info.get("apos_V", 0), "Após → Preto": info.get("apos_P", 0), "Números que quebraram": {str(num_k): qtd_v for num_k, qtd_v in info.get("quebradores", {}).items()}, "G0 (Acerto Direto)": info.get("g0", 0), "G1 (Acerto no G1)": info.get("g1", 0), "Assertividade Real %": round(((info.get("g0", 0) + info.get("g1", 0)) / info.get("total", 1)) * 100, 1)})
                                else: st.info("Nenhum padrão geral dinâmico ou espelho encontrado.")
                    else: st.warning(relatorio.get("mensagem"))
                else: st.error("Não foi possível ler os dados do arquivo.")
            except Exception as e: st.error(f"Erro ao salvar e treinar base: {e}")
            if os.path.exists(caminho_temp): os.remove(caminho_temp)
                    ...
                    if relatorio.get("sucesso"):
                        if relatorio.get("modelo_salvo_com_sucesso"):
                            ...
                            with st.expander("♟️ Padrões Avançados Aprendidos nesta Base (Xadrez e Streak) + Métricas de Assertividade", expanded=False):
                                ...
                                else: st.error("Modelo ainda não foi salvo corretamente. Use o botão 'Substituir Base de Longo Prazo'.")
                        ...
                        else: st.error("Treinamento realizado, mas falha ao salvar o modelo. Tente usar 'Substituir Base de Longo Prazo'.")
                    else: st.warning(relatorio.get("mensagem"))
                else: st.error("Não foi possível ler os dados do arquivo.")
            except Exception as e: st.error(f"Erro ao adicionar dados à base: {e}")
            if os.path.exists(caminho_temp): os.remove(caminho_temp)
...
with aba_padroes:
    ...
    if st.button("🔄 Carregar Padrões do Modelo Atual"):
        ...
        if ia is None: st.warning("Nenhum modelo de longo prazo encontrado. Treine uma base primeiro.")
        else:
                ...
                if hasattr(ia, 'padroes_xadrez_detalhado') and ia.padroes_xadrez_detalhado:
                    for padrao, info in ia.padroes_xadrez_detalhado.items():
                        if info.get("total", 0) >= 5:
                            st.markdown(f"**{padrao}** — {info.get('total')}x")
                            st.json({"Após → Vermelho": info.get("apos_V", 0), "Após → Preto": info.get("apos_P", 0), "Números que trollam": {str(num_k): qtd_v for num_k, qtd_v in info.get("quebradores", {}).items()}, "G0": info.get("g0", 0), "G1": info.get("g1", 0), "Assertividade %": round(((info.get("g0", 0) + info.get("g1", 0)) / info.get("total", 1)) * 100, 1)})
                else: st.info("Sem padrões de Xadrez registrados.")
                ...
                if hasattr(ia, 'padroes_streak_detalhado') and ia.padroes_streak_detalhado:
                    for padrao, info in ia.padroes_streak_detalhado.items():
                        if info.get("total", 0) >= 5:
                            st.markdown(f"**{padrao}** — {info.get('total')}x")
                            st.json({"Após → Vermelho": info.get("apos_V", 0), "Após → Preto": info.get("apos_P", 0), "Números que trollam": {str(num_k): qtd_v for num_k, qtd_v in info.get("quebradores", {}).items()}, "G0": info.get("g0", 0), "G1": info.get("g1", 0), "Assertividade %": round(((info.get("g0", 0) + info.get("g1", 0)) / info.get("total", 1)) * 100, 1)})
                else: st.info("Sem padrões de Streak registrados.")
                ...
                if hasattr(ia, 'padroes_gerais_detalhado') and ia.padroes_gerais_detalhado:
                    for padrao, info in ia.padroes_gerais_detalhado.items():
                        if info.get("total", 0) >= 5:
                            st.markdown(f"**{padrao}** — {info.get('total')}x")
                            st.json({"Após → Vermelho": info.get("apos_V", 0), "Após → Preto": info.get("apos_P", 0), "Números que quebraram": {str(num_k): qtd_v for num_k, qtd_v in info.get("quebradores", {}).items()}, "G0 (Acerto Direto)": info.get("g0", 0), "G1 (Acerto no G1)": info.get("g1", 0), "Assertividade Real %": round(((info.get("g0", 0) + info.get("g1", 0)) / info.get("total", 1)) * 100, 1)})
                else: st.info("Sem padrões dinâmicos ou espelhos registrados.")
